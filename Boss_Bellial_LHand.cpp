...
#include "NormalEffect_Hit.h"
#include "Boss_Bellial_Laser.h"
CBoss_Bellial_LHand::CBoss_Bellial_LHand()
	:m_eCurState(Idle)
	, m_eNextState(Idle)
	, m_fSpeed(0.f)
	, m_fLineLX(0.f)
...
int CBoss_Bellial_LHand::Update()
{
	if (m_bDead)
		return DEAD_OBJ;
...
void CBoss_Bellial_LHand::LateUpdate()
{

}

void CBoss_Bellial_LHand::Render()
{
...
HRESULT CBoss_Bellial_LHand::Initialize(float fX, float fY)
{
...
void CBoss_Bellial_LHand::MonsterWalk()
{
	//float fY = 0.f;
	...
	if (5000.f > fDist)
	{
		...
		if (4.7f <= m_fDelayTimeTwo)
		{
			...
			if (5.1f <= m_fDelayTimeTwo)
			{
				...
				// Ãß»óÃþ
				CNormalEffect_Hit* pEffect = CNormalEffect_Hit::Create(
					pDash, D3DXVECTOR3(m_tInfo.vPos.x +55, m_tInfo.vPos.y + 25 , 0.f));

				CObjectMgr::GetInstance()->AddObject(CGameObject::EFFECT, pEffect);

				CGameObject* pInstance = CBoss_Bellial_Laser::Create(D3DXVECTOR3(m_tInfo.vPos+ m_tInfo.vDir), m_tInfo.vDir, m_fAngle, -(m_tInfo.vSize.x / fabsf(m_tInfo.vSize.x)), 3.f);
...
void CBoss_Bellial_LHand::StateChange()
{
	if (m_eCurState != m_eNextState)
	{
		switch (m_eNextState)
		{
		case CBoss_Bellial_LHand::Idle:
			m_tFrame.fCurFrame = 0.f;
			m_tFrame.fMaxFrame = 9.f;
			m_pFrameKey = L"HandIdle";
			break;
		case CBoss_Bellial_LHand::Attack:
			m_tFrame.fCurFrame = 0.f;
			m_tFrame.fMaxFrame = 18.f;
			m_pFrameKey = L"HandAttack";
			break;
			//case CBoss_Bellial_LHand::Dead:
			//	m_tFrame.fCurFrame = 0.f;
			//	m_tFrame.fMaxFrame = 9.f;
			//	m_pFrameKey = L"Dead";
			//	break;
...
void CBoss_Bellial_LHand::MakeWorldMatrix()
{
...
HRESULT CBoss_Bellial_LHand::Initialize()
{
	return S_OK;
}

HRESULT CBoss_Bellial_LHand::LateInit()
{
...
void CBoss_Bellial_LHand::Release()
{
}

void CBoss_Bellial_LHand::MoveFrame(float fStartFrame, float fMaxFrame, float fSpeed)
{
...
CBoss_Bellial_LHand * CBoss_Bellial_LHand::Create()
{
	return new CBoss_Bellial_LHand;
}

void CBoss_Bellial_LHand::LineRender()
{
}
...
void CBoss_Bellial_LHand::SetHandAngle()
{
	//POINTa pt = {};
	//GetCursorPos(&pt);
